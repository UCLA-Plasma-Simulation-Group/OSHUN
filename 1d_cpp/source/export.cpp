...
//--------------------------------------------------------------
        ...
        if ( Input::List().o_x1x2 ||  Input::List().o_pth ||
             ...
             Input::List().o_Vsq || Input::List().o_Temperature || Input::List().o_Pressure || Input::List().o_Qx || Input::List().o_Qy || Input::List().o_p1x1 )  {
...
//><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><-><
        ...
        if ( Input::List().o_f0x1 || Input::List().o_p1x1 ||  Input::List().o_f10x1 ||  Input::List().o_f11x1) {
            ...
            //         cout<<"Warning: Folder 'OUTPUT/DISTR/P1X1' exists" << endl;
            // }
            // if (Input::List().o_f0x1) { 
            //     if (Makefolder("OUTPUT/DISTR/F0X1") != 0) 
            //         cout<<"Warning: Folder 'OUTPUT/DISTR/F0X1' exists" << endl;
            // }
        }
...
//          For each 9 you have a different species 
            Hdr[dTags.pvsx[i]] = Header( pxyz[i], xyz[0], 
...
//  Generate the required structures
    for (size_t s(0); s < _G.axis.pdim(); ++s) {
        ...
        Pl.push_back( PLegendre1D( _G.Nl[s], _G.Nm[s], _G.axis.Np(s), static_cast<float>(_G.axis.pmin(s)), 
...
//  Generate the required structures
    for (size_t s(0); s < _G.axis.pdim(); ++s) {
        pmax.push_back( static_cast<float>(_G.axis.pmax(s)) );
        pmin.push_back( static_cast<float>(_G.axis.pmin(s)) );
        numl.push_back( _G.Nl[s]);
        numm.push_back( _G.Nm[s]);

        Pl.push_back( PLegendre2D( _G.Nl[s], _G.Nm[s], _G.axis.Np(s), static_cast<float>(_G.axis.pmin(s)), 
...
void Output_Data::Output_Preprocessor_1D::px(const State1D& Y, const Grid_Info& grid, const size_t tout,
        ...
        for(int s(0); s < Y.Species(); ++s) {     
            ...
            int msg_sz(outNxLocal*px_x.Npx(s)); 
            Array2D<float> p1x1Global(px_x.Npx(s),outNxGlobal); //, yglob_axis.dim());
            float* pxbuf = new float[px_x.Npx(s)*outNxLocal];
            
            for (size_t i(0); i < outNxLocal; ++i) {
                  
                valarray<float> data1D = px_x( Y.DF(s), i+Nbc, s);
                
                for (size_t j(0); j < px_x.Npx(s); ++j) {
                    pxbuf[j+i*px_x.Npx(s)]=data1D[j];
                }
            }
            ...
            if (PE.NODES() > 1) { 
                if (PE.RANK()!=0) {
                   MPI_Send(pxbuf, msg_sz, MPI_FLOAT, 0, PE.RANK(), MPI_COMM_WORLD);
                    ...
                    for(size_t i(0); i < outNxLocal; i++) {
                        for (size_t j(0); j < px_x.Npx(s); ++j) {
                            p1x1Global(j,i) = pxbuf[j+i*px_x.Npx(s)];
                        }
                    }
                    // Fill data for rank > 0 
                    for (int rr = 1; rr < PE.NODES(); ++rr){
                        ...
                        for(size_t i(0); i < outNxLocal; i++) {
                            for (size_t j(0); j < px_x.Npx(s); ++j) {
                                p1x1Global(j,i + outNxLocal*rr) = pxbuf[j+i*px_x.Npx(s)];
                            }
                        }
                    }
                 }
...
void Output_Data::Output_Preprocessor_1D::vNx(const State1D& Y, const Grid_Info& grid, const size_t tout,
        ...
        for(int s(0); s < Y.Species(); ++s) {
            ...
            for(size_t i(0); i < msg_sz; ++i) {
                vNxbuf[i] = 1.0/6.0*static_cast<float>(Algorithms::moment(  vfloat( (Y.SH(s,1,0)).xVec(i+Nbc) ), pra, 6)
...
void Output_Data::Output_Preprocessor_1D::Ux(const State1D& Y, const Grid_Info& grid, const size_t tout,
        ...
        // for(int s(0); s < Y.Species(); ++s) {
            ...
            for(size_t i(0); i < msg_sz; ++i) {
                Uxbuf[i] = static_cast<float>(Y.HYDRO().velocity(i+Nbc));
            }
...
//  Open File
        ...
        float xmin = {  static_cast<float>( Hdr[tag].axis(0)[ 0 ] ) };
        float xmax = {  static_cast<float>( Hdr[tag].axis(0)[ (Hdr[tag].axis(0).size()-1) ] ) };
...
//  Open File
        ...
        float xmin[2] = {   static_cast<float>( Hdr[tag].axis(0)[ 0 ] ),
                            static_cast<float>( Hdr[tag].axis(1)[ 0 ] ) };
        
        float xmax[2] = {   static_cast<float>( Hdr[tag].axis(0)[ (Hdr[tag].axis(0).size()-1) ] ),
...
//  Open File
        ...
        float xmin[3] = {   static_cast<float>( Hdr[tag].axis(0)[ 0 ] ),
                            static_cast<float>( Hdr[tag].axis(1)[ 0 ] ),
                            static_cast<float>( Hdr[tag].axis(2)[ 0 ] ) };
        float xmax[3] = {   static_cast<float>( Hdr[tag].axis(0)[ (Hdr[tag].axis(0).size()-1) ] ),
                            static_cast<float>( Hdr[tag].axis(1)[ (Hdr[tag].axis(1).size()-1) ] ),

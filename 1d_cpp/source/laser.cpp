...
//-------------------------------------------------------------------
    valarray<double>& IB_f00::Getslope(const valarray<double>& fin, const double vos) {
//-------------------------------------------------------------------
...
//     Evaluate the integrals in a standard manner
        ...
        ZLn_ei = formulas.Zeta*formulas.LOGei(4.0*M_PI*I2,I4/3.0/I2,formulas.Zeta);
...
//  Call Advection Actions 
    valarray<double>& RK4_IB::F(const valarray<double>& fin, const double vosc) { 
...
//       -----
         ...
         IL_xprofile(numx)
         // IL_yprofile(Y.SH(0,0).numy()),
         // xaxis(Input::List().x),
         // rise_time(Input::List().rise_time),
         // fall_time(Input::List().fall_time),
         // flat_time(Input::List().flat_time)
         {
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
//       integrate over x you get I0(t)
        ...
        // szy = Y.SH(0,0).numy();      // size of useful y axis

        // Calculate the intensity normalization coefficient
        // double sumI(0.0);
        // for (size_t i(0); i < Input::List().xglob.dim(); ++i){
	//    sumI += exp((-1.0)*pow((Input::List().xglob(i)-Input::List().cntr_x0)/Input::List().ab_depth,2));
        // }
       ...
       for (size_t i(1); i < U2.size(); ++i) {
        ...
        // for (size_t i(0); i < Y.SH(0,0).numx(); ++i){
            // IL_xprofile[i] = exp((-1.0)*pow((Input::List().x(i)-Input::List().cntr_x0)/Input::List().ab_depth,2));
        // }
        //  IL_xprofile *= 1.0/sumI;

        
        // for (size_t j(0); j < Y.SH(0,0).numy(); ++j){
        //     double tmp2 = ( Input::List().y(j) - Input::List().cntr_r0 ) / Input::List().spot_w0;
...
//--------------------------------------------------------------
    void InverseBremsstrahlung::loop(SHarmonic1D& f0, const double& tnew){ 
//--------------------------------------------------------------

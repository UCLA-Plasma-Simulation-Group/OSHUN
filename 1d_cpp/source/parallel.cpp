...
//  Constructor
//--------------------------------------------------------------
        Nbc(Input::List().RKLevel),      // # of boundary cells
...
//--------------------------------------------------------------
        ...
        if (Input::List().hydromotion)
        {
            ...
            // Hydro-velocity:   x0 "Right-Bound --> "
    		for(int e(0); e < Nbc; e++) {
    			msg_bufX[bufind + e] = Y.HYDRO().velocity(Y.FLD(0).numx()-2*Nbc+e);
    		} 
    		bufind += step_f;
...
//--------------------------------------------------------------
        ...
        if (Input::List().hydromotion)
        {
    		...
    		for(int e(0); e < Nbc; e++) {
    			Y.HYDRO().velocity(Y.FLD(0).numx()-Nbc+e) = (msg_bufX[bufind + e]).real();
    		} 
    		bufind += step_f;
...
//--------------------------------------------------------------
        ...
        if (Input::List().hydromotion)
        {
            ...
            for(int c(0); c < Nbc; c++) {
                Y.HYDRO().velocity(Y.EMF().Ex().numx()-Nbc+c) *=  -1.0;
            }
        }
    ...
    //     }
        ...
        if (Input::List().hydromotion)
        {
			...
			for(int c(0); c < Nbc; c++) {
                ...
                Y.HYDRO().velocity(c) = Y.HYDRO().velocity(Y.EMF().Ex().numx()-2*Nbc+c);
			...
			for(int c(0); c < Nbc; c++) {
                ...
                Y.HYDRO().velocity(Y.EMF().Ex().numx()-Nbc+c) =  Y.HYDRO().velocity(Nbc+c);
...
//--------------------------------------------------------------
        ...
        //     for(int c(0); c < Nbc; c++) {
        //         Y.HYDRO().velocity(c) *= -1.0;
        //         // Y.HYDRO().temperature(c) = Y.HYDRO().velocity(Y.EMF().Ex().numx()-2*Nbc+c);
        //         // Y.HYDRO().kpressure(c) = Y.HYDRO().velocity(Y.EMF().Ex().numx()-2*Nbc+c);
        //         // Y.HYDRO().mpressure(c) = Y.HYDRO().velocity(Y.EMF().Ex().numx()-2*Nbc+c);
        //     }    
        ...
        //     // Hydro Quantities:   x0 "Left-Bound ---> Right-Guard"
        //     for(int c(0); c < Nbc; c++) {
        //         Y.HYDRO().velocity(Y.EMF().Ex().numx()-Nbc+c) *= -1.0;// Y.HYDRO().velocity(Nbc+c);
        //         // Y.HYDRO().temperature(Y.EMF().Ex().numx()-Nbc+c) =  Y.HYDRO().velocity(Nbc+c);
        //         // Y.HYDRO().kpressure(Y.EMF().Ex().numx()-Nbc+c) =  Y.HYDRO().velocity(Nbc+c);
        //         // Y.HYDRO().mpressure(Y.EMF().Ex().numx()-Nbc+c) =  Y.HYDRO().velocity(Nbc+c);
...
//         restart_time(Input::List().restart_time)
		...
		for(size_t i(0); i < Input::List().xminLocal.size(); ++i) {
			...
			Input::List().xminLocal[i] = Input::List().xminGlobal[i]
										...
										- Input::List().RKLevel * Input::List().globdx[i];
...
//--------------------------------------------------------------
        ...
        if (rank == 0){ 
            if (Input::List().numx < 2*Input::List().RKLevel+1){
                std::cout << "Not enough cells per processor" << endl;
                return true;
            }
            if ( Nnodes != (Input::List().NnodesX * Input::List().NnodesY) ) { 
                std:: cout << "the number of nodes in the input deck is "
                << (Input::List().NnodesX * Input::List().NnodesY) << ", terminating ..." << endl;
                return true; 
            }
         } 
         return false;

...
void Current_1D::operator()(const DistFunc1D& Din, Field1D& Exh, Field1D& Eyh, Field1D& Ezh) {
    ...
    SHarmonic1D f10( Din(1,0) );
    f10 = f10.mpaxis( invg );

    temp = Din.getcurrent();

    for (size_t i(0); i < Jx.numx(); ++i) {
        Jx(i) = static_cast<complex<double>>(temp(0,i));
        Jy(i) = static_cast<complex<double>>(temp(1,i));
        Jz(i) = static_cast<complex<double>>(temp(2,i));
    }
...
//--------------------------------------------------------------
  : tmpE(Nx), numx(Nx) {
		...
		idx = static_cast<complex<double> >((-1.0)/ (2.0*(xmax-xmin)/double(Nx))); // -1/(2dx) 
...
//--------------------------------------------------------------
    ...
    for (size_t s(0); s < Nl.size(); ++s){ 
        ...
        SA.push_back( Spatial_Advection_1D(Nl[s], Nm[s], pmin, pmax[s], Np[s], xmin, xmax, Nx) );

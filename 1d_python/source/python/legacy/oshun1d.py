...
class WaveDrivah:
	def __init__(self, dt, species, num_wavelengths):
		pass
		...
		self.num_wavelengths = 3 # num_wavelengths
		...
		self.discrete_factor = np.pi*2*float( self.num_wavelengths)/spatial_axis_global.values.shape[0]
	...
	def apply(self, E_field, spatial_axis_global, spatial_axis__local_in_global, global_cell_offset, buffah=None, phase_me = 0.0):
		...
		if self.num_wavelengths != None:
			...
			if buffah != None:
				data = buffah.components[0]
			for (n,x_val) in enumerate(spatial_axis__local_in_global.values):
				global_cell_index = global_cell_offset[0] + n
...
def main_loop():
	...
	DaWaveDrivah = WaveDrivah(STATE.config.dt, STATE.species[0], 1.0)
			...
			if((n%50) == 0):
				...
				if i == 0:
					...
					if mpi_gather.gather( STATE.E_field.components[0], remove_guard_cells=True):
						clf()
						subplot(211)
						plot(mpi_gather.stiching_buffer)
						ylim( [-0.0015, 0.0015] )
						axhline( y = 0)
						...
						size -= 1.0
						stem(size,np.absolute(fft.fftshift(fft.fft(mpi_gather.stiching_buffer))))
						ylim( [0.0, .04] )
				...
				# the daignostics emmited from Moments already have the gaurd cells trimmed
				if mpi_gather.gather(  moments[i].get_density(), remove_guard_cells=True ):
					clf(); plot(mpi_gather.stiching_buffer); title("Density %.02f, time %.02e" % (n, time));
					draw(); savefig("./png/%sdensity_%06d.png" % (species_name_prefix, n) )
...
def ld_setup():
	...
	num_species						=  1
	num_cells__global[0] 		=  192  #96 # worked nice
	system_size_min__global[0] =  0.0
	system_size_max__global[0] =  1.0
	CFG = Config( num_cells__global, system_size_min__global, system_size_max__global, num_species)
	...
	CFG.num_l 						= 24
	CFG.num_m 						= 1
	CFG.dt 							= 0.01 		#0.005 #.02 #299383 #.01
	CFG.t_end 						= 3880.0
	
	CFG.abs_p_max 					= .15 			# .75 #.06 #.15 #0.15
	CFG.abs_p_num_cells 			= 96 			#150
	CFG.abs_p_min					= CFG.abs_p_max/(2.0 * CFG.abs_p_num_cells)
	CFG.explicit_solver_order 	= 3
...
def setup():
	...
	# C stuff...
	print STATE.spatial_axes[0].num_points
	print type(STATE.spatial_axes[0].num_points)

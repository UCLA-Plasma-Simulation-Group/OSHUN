...
def plotme(hdf_data, data = None):
	...
	if len(data_to_use.shape) == 1:
		plot_object =  ppyplot.plot(hdf_data.axes[0].get_axis_points(), data_to_use)
		pyplot.xlabel(hdf_data.axes[0].attributes['LONG_NAME'] + "\n" + hdf_data.axes[0].attributes['UNITS'])
		pyplot.ylabel(hdf_data.data_attributes['LONG_NAME'] + "\n" + hdf_data.data_attributes.attributes['UNITS'])
		return plot_object
	...
	if len(data_to_use.shape) == 2:
		...
		plot_object = pyplot.imshow(data_to_use, extent=extent_stuff, aspect='auto', cmap='Rainbow')
		pyplot.colorbar(plot_object)
		pyplot.xlabel("%s \n %s" % (hdf_data.axes[0].attributes['LONG_NAME'], hdf_data.axes[0].attributes['UNITS'] ))
...
class hdf_data:
	...
	def slice(self, x=None, y=None, z=None, copy=False):
		slice_index_array = []
		target_obj = self
		...
		for axis in temp_axes:
			...
			if axis.axis_number == 1:
				selection = self.__slice_dim(x, self.data, 1)
			if axis.axis_number == 2:
				selection = self.__slice_dim(y, self.data, 2)
			if axis.axis_number == 3:
				selection = self.__slice_dim(z, self.data, 3)
...
def read_hdf(filename):
	...
	Usage:
			...
			print diag_data.run_attributes['TIME']		# prints the simulation time associates with the hdf5 file
			diag_data.data_attributes['UNITS']     		# print units of the dataset points
			list(diag_data.data_attributes)         		# lists all vairables in 'data_attributes'
...
def __write_axis_helper( axis, amin, amax, anum, axis_num):
	if axis != None:
		data_x_axis = data_basic_axis(0, axis[0] , axis[-1], axis.shape[0] )
	else:
		if amin == None:
			# error
			return
		...
		if anum == None:
			return
		data_x_axis = data_basic_axis(0, amin, amax, anum )
	return data_x_axis

def write_hdf_dataset_1d( data, filename, x_axis_vector = None, xaxis_min = None, xaxis_max = None, xaxis_number_points = None,
														y_axis_vector = None, yaxis_min = None, yaxis_max = None, yaxis_number_points = None, dataset_name = None):
	
	data_x_axis = __write_axis_helper ( x_axis_vector, xaxis_min, xaxis_max, xaxis_number_points, 0)
	if data_x_axis == None:
		return
	data_y_axis = __write_axis_helper ( y_axis_vector, yaxis_min, yaxis_max, yaxis_number_points, 1)
	if data_y_axis == None:
		minn = np.min(data); maxx = np.max(data); 
		data_y_axis =  data_basic_axis(1, minn, maxx, data.shape[0] )
	
	
	data_object = hdf_data()
	data_object.data = data
	data_object.axes.append(data_x_axis); data_object.axes.append(data_y_axis);
	write_hdf ( data_object, filename, dataset_name = dataset_name, write_data = True)

def write_hdf_dataset_2d( data, filename, x_axis_vector = None, xaxis_min = None, xaxis_max = None, xaxis_number_points = None,
														y_axis_vector = None, yaxis_min = None, yaxis_max = None, yaxis_number_points = None,
														data_axis_vector = None, data_min = None,data_max = None,data_number_points = None,dataset_name = None):
	...
	data_y_axis = __write_axis_helper ( y_axis_vector, yaxis_min, yaxis_max, yaxis_number_points, 1)
	if data_y_axis == None:
		data_y_axis =  data_basic_axis(1, np.min(data), np.max(data), data.shape[0] )
		return
	if data_data_axis == None:
		data_data_axis =  data_basic_axis(1, np.min(data), np.max(data), 100 )
	
	data_obj = hdf_data()
	data_obj.data = data
	data_obj.axes.append(data_x_axis); data_obj.axes.append(data_y_axis);
	write_hdf ( data_obj, filename, dataset_name = dataset_name, write_data = True)
...
def write_hdf(data, filename, dataset_name = None, write_data = True):
	...
	h5dataset = file.create_dataset(current_NAME_attr, data_object.data.shape)
	...
	for i in range(0, number_axis_object_present):
		...
		if axis_name in file:
			if i<number_axis_objects_we_need:
				...
				del file[axis_name]
	# now go tourgh and set/create our axes HDF entries.

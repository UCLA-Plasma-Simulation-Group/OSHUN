...
def addToZip( zipFilename, filepathToAdd, destDirPrefix = None, destFilename = None):
	...
	if( os.path.isdir(filepathToAdd)  ):
		...
		if( destDirPrefix != None and destFilename != None ):
			...
			outputFilename = os.path.join( destDirPrefix, os.path.basename(filepathToAdd))
...
if __name__ == "__main__":
	...
	if  len(sys.argv) < 3:
		print "Wrong number of arguments provided.. the format of the command is: python AddFilesToZip zipname filepath destinationPrefix destinationFilename"
		exit(-1)
	...
	addToZip( sys.argv[1], sys.argv[2], destinationDir, destinationFilename )

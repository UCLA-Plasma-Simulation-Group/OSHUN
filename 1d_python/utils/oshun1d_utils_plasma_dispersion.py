...
try:
	...
	print "The import of matplotlib.pyplot was unsuccessful."
	print "You risk trouble making plots without importing matplotlib.pyplot"


def plasma_dispersion( value):
	# wofz(z)   = exp(-z**2)*erfc(-1j*z)
	#           = exp(-z**2)*[1+erf(1j*z)]
...
def landau_damping_frequency(wp_e, vth__e, wavenumber, maxwellian_convention_factor = 2.0, inital_root_guess = None ):
	...
	#
	#		The default inital_root_guess is z = w/(sqrt(2)*wavenumber*vth__e) with w = sqrt(wp_e^2 + 3*wavenumber^2*vth__e^2)
	#
	# example:
	#					wp_e = 1.0						
	#					vth_e = sqrt( .1 )
	...
	#														
	# NOTE: if wp_e and/or vth__e does not equal 1, your normalization will carry through the solver
	#
	...
	if inital_root_guess == None:
		...
		# guess for w must be converted to z = w/(sqrt(2)*wavenumber*vth__e)
		inital_root_guess /= wavenumber * vth__e * np.sqrt(maxwellian_convention_factor)
...
def landau_spatial_damping( wp_e, vth__e, w_wave, maxwellian_convention_factor = 2.0, inital_root_guess = None ):
	...
	if inital_root_guess == None:
		# use the Bohm-Gross dispersion formulas to get an initial guess for k
		inital_root_guess = np.sqrt( (np.power(w_wave,2.0) - np.power(wp_e,2.0)) / ( 3.0*np.power(vth__e,2.0) ))
		# guess for k must be converted to z = w_wave/(sqrt(2)*k*vth__e)
		inital_root_guess = exp_factor / inital_root_guess
...
def test__show_dispersion_relation( wp__e, vth__e, k_min, k_max, k_steps):
	...
	plt.clf()
	plt.plot(k__vthe,w_real, label='Plasma Dispersion Calculation' )
	plt.plot(k__vthe,bohm_gross_freqs, label='Bohm-Gross Relation' )
	plt.xlabel( '$V_{thermal}*Wavenumber$ ( units of inverse skindepth)')
	plt.ylabel( 'Real frequency (units of $w_p$)' )
	plt.title('Nominal Dispersion Relation\nElectrostatic (Kinetic) Dielectric')
	plt.legend(loc=2, prop={'size':10})
	plt.xlim( ( 0.0, k__vthe[-1] ) ); 
	plt.ylim( ( 0.0, 2.0) );
	plt.savefig('dispersion__real_freq__kinetic_electrostatic_dielectrix', dpi=200)
	
	plt.clf()
	plt.plot( k__vthe, w_imag*-1.0, label='Plasma Dispersion Calculation' )
	plt.plot( k__vthe, krall_trivelpiece_damping, label='Krall-Trivelpiece approx. result' )
	plt.title('Nominal Dispersion Relation\nElectrostatic (Kinetic) Dielectric (Damping)')
	plt.xlabel( '$V_{thermal}*Wavenumber$ ( units of inverse skindepth)')
	plt.ylabel( '-1*Imag frequency (units of $w_p$)' )
	plt.legend(loc=2, prop={'size':10})
	plt.xlim( ( 0.0, k__vthe[-1] ) )
	plt.axhline( y=0, color='black')
	plt.savefig('dispersion__imag_freq__kinetic_electrostatic_dielectrix', dpi=200)
	
	# log versions...
	plt.clf()
	plt.plot( k__vthe, np.log( ( -1.0*w_imag ) ), label='Plasma Dispersion Calculation'  )
	plt.plot( k__vthe, np.log( krall_trivelpiece_damping ), label='Krall-Trivelpiece approx. result' )
	plt.axvline( x=.105, color='black', alpha=.5, label="Lower Limit Allowed in Code")
	plt.title('Nominal Dispersion Relation\nElectrostatic (Kinetic) Dielectric (Damping)')
	plt.xlabel( '$V_{thermal}*Wavenumber$ ( units of inverse skindepth)')
	plt.ylabel( '$log_{e}$  of -1*Imag frequency (units of $w_p$)' )
	plt.xlim( ( 0.0, k__vthe[-1] ) )
	plt.ylim( ( -40.0, 3.0) )
	plt.axhline( y=0, color='black')
	plt.legend(loc=4, prop={'size':10})
	plt.savefig('dispersion__imag_freq__kinetic_electrostatic_dielectrix__log', dpi=200)
	plt.clf()
	plt.plot( k__vthe, np.log10( ( -1.0*w_imag ) ), label='Plasma Dispersion Calculation' )
	plt.plot( k__vthe, np.log10( krall_trivelpiece_damping ), label='Krall-Trivelpiece approx. result' )
	plt.axvline( x=.105, color='black', alpha=.5, label="Lower Limit Allowed in Code")
	plt.title('Nominal Dispersion Relation\nElectrostatic (Kinetic) Dielectric (Damping)')
	plt.xlabel( '$V_{thermal}*Wavenumber$ ( units of inverse skindepth)')
	plt.ylabel( '$log_{10}$ of -1*Imag frequency (units of $w_p$)' )
	plt.xlim( ( 0.0, k__vthe[-1] ) )
	plt.ylim( ( -20.0, 1.0) )
	plt.axhline( y=0, color='black')
	plt.legend(loc=4, prop={'size':10})
	plt.savefig('dispersion__imag_freq__kinetic_electrostatic_dielectrix__logten', dpi=200)
	...
	#print k__vthe
	#print k__vthe2
	plt.plot(k__vthe2,w_real, label='Plasma Dispersion Calculation' )
	plt.plot(k__vthe2,bohm_gross_freqs, label='Bohm-Gross Relation' )
	plt.xlabel( '$V_{thermal}*Wavenumber$ squared ( units of inverse skindepth)')
	plt.ylabel( 'Real frequency (units of $w_p$)' )
	plt.title('Nominal Dispersion Relation\nElectrostatic (Kinetic) Dielectric')
	plt.legend(loc=2, prop={'size':10})
	plt.xlim( ( 0.04, .12 ) ); plt.ylim( ( 1.05, 1.25) );
	plt.savefig('dispersion__real_freq__kinetic_electrostatic_dielectrix__squared_x', dpi=200)
	
	plt.clf()
	plt.plot( k__vthe2, np.log10( w_imag*-1.0 ), label='Plasma Dispersion Calculation' )
	plt.plot( k__vthe2, np.log10(krall_trivelpiece_damping), label='Krall-Trivelpiece approx. result' )
	plt.xlabel( '$V_{thermal}*Wavenumber$ squared ( units of inverse skindepth)')
	plt.ylabel( 'Real frequency (units of $w_p$)' )
	plt.title('Nominal Dispersion Relation\nElectrostatic (Kinetic) Dielectric')
	plt.legend(loc=2, prop={'size':10})
	plt.xlim( ( 0.04, .12 ) ); plt.ylim( ( -5.0, -1.0) );
	plt.savefig('dispersion__imag_freq__kinetic_electrostatic_dielectrix__squared_x', dpi=200)
...
def plot_epilsion(wp_e, vth__e, wavenumber, maxwellian_convention_factor = 2.0):
	...
	plt.clf()
	plt.imshow( np.log10( result) ,interpolation='nearest', extent=[bounds_real[0], bounds_real[-1],bounds_imag[0], bounds_imag[-1]], aspect='auto')
	plt.axvline( x = wr); 
	plt.axhline( y = wi); 
	plt.colorbar()
	plt.savefig('aaaa', dpi=400)
...
def test_plasma_dispersion():
	...
	plt.clf()
	ax = plt.subplot(111)
	plt.title("$\\Re(Z)$ and $\\Re(Z')$ versus $x$ for $y=0$")
	plt.xlabel("$x$")
	plt.ylabel("$\\Re(Z)$,$\\Re(Z')$ ")
	plt.plot( x_values, pd__real_values, label="$\\Re(Z)$")
	plt.plot( x_values, pd_prime__real_values, label="$\\Re(Z')$")
	plt.axhline( y=0, color='black')
	plt.legend( loc=4)
	plt.ylim( (-2.0, .8) )
	ax.set_yticks(np.arange(-2.0,1.2,0.4))
	plt.grid()
	plt.savefig("pd_test__fig_01_real_y_0.png")
	
	plt.clf()
	ax = plt.subplot(111)
	plt.title("$\\Im(Z)$ and $\\Im(Z')$ versus $x$ for $y=0$")
	plt.xlabel("$x$")
	plt.ylabel("$\\Im(Z)$,$\\Im(Z')$ ")
	plt.plot( x_values, pd__imag_values,label="$\\Im(Z)$")
	plt.plot( x_values, pd_prime__imag_values, label="$\\Im(Z')$")
	plt.axhline( y=0, color='black')
	plt.legend( loc=4)
	plt.ylim( (-1.6, 1.8) )
	plt.xlim( (0.0, 5.0) )
	ax.set_yticks(np.arange(-1.6,2.0,0.4))
	plt.grid()
	plt.savefig("pd_test__fig_02_imag_y_0.png")
	...
	plt.clf()
	ax = plt.subplot(111)
	plt.title("$\\Re(Z)$ versus $x$ for $y=1,2,5,10$")
	plt.xlabel("$x$")
	plt.ylabel("$\\Re(Z)$")
	plt.plot( x_values, pd_r__01, label='y=1'); 
	plt.plot( x_values, pd_r__02, label='y=2'); 
	plt.plot( x_values, pd_r__05, label='y=5'); 
	plt.plot( x_values, pd_r__10, label='y=10')
	plt.axhline( y=0, color='black')
	plt.legend( loc=4)
	plt.ylim( (-0.425, 0.0) )
	ax.set_yticks(np.arange(-0.4,0.05,0.05))
	plt.grid()
	plt.savefig("pd_test__fig_03_real_z_positive_y.png")
	
	plt.clf()
	ax = plt.subplot(111)
	plt.title("$\\Im(Z)$ versus $x$ for $y=1,2,5,10$")
	plt.xlabel("$x$")
	plt.ylabel("$\\Im(Z)$")
	plt.plot( x_values, pd_i__01, label='y=1'); 
	plt.plot( x_values, pd_i__02, label='y=2'); 
	plt.plot( x_values, pd_i__05, label='y=5'); 
	plt.plot( x_values, pd_i__10,label='y=10')
	plt.axhline( y=0, color='black')
	plt.legend( loc=1)
	plt.ylim( (0.0, 0.8) )
	ax.set_yticks(np.arange(0.0,0.9,0.1))
	plt.grid()
	plt.savefig("pd_test__fig_04_imag_z_positive_y.png")
	...
	plt.xlabel("$x$")
	plt.ylabel("$\\Re(Z')$")
	plt.plot( x_values, pdp_r__01,label='y=1'); 
	plt.plot( x_values, pdp_r__02,label='y=2'); 
	plt.plot( x_values, pdp_r__05,label='y=5'); 
	plt.plot( x_values, pdp_r__10,label='y=10')
	plt.axhline( y=0, color='black')
	plt.legend( loc=4)
	plt.ylim( (-0.5, 0.2) )
	ax.set_yticks(np.arange(-0.5,0.3,0.1))
	plt.grid()
	plt.savefig("pd_test__fig_05_real_zprime_positive_y.png")
	...
	plt.xlabel("$x$")
	plt.ylabel("$\\Im(Z')$")
	plt.plot( x_values, pdp_i__01,label='y=1'); 
	plt.plot( x_values, pdp_i__02,label='y=2'); 
	plt.plot( x_values, pdp_i__05,label='y=5'); 
	plt.plot( x_values, pdp_i__10,label='y=10')
	plt.axhline( y=0, color='black')
	plt.legend( loc=4)
	plt.ylim( (-0.35, 0.0) )
	ax.set_yticks(np.arange(-0.35,0.05,0.05))
	plt.grid()
	plt.savefig("pd_test__fig_06_imag_zprime_positive_y.png")
	
	# do figs 7 and 8
	(  x_values, pd_r__m1, pdp_r__m1, pd_i__m1, pdp_i__m1) = test_plasma_dispersion__generate_values_for_a_given_y(-1.0)
	plt.clf()
	ax = plt.subplot(111)
	plt.title("$\\Re(Z)$ and $\\Im(Z)$ versus $x$ for $y=-1.0$")
	plt.xlabel("$x$")
	plt.ylabel("$\\Re(Z)$,$\\Im(Z)$ ")
	plt.plot( x_values, pd_r__m1, label="$\\Re(Z)$"); 
	plt.plot( x_values, pd_i__m1, label="$\\Im(Z)$");
	plt.axhline( y=0, color='black')
	plt.legend( loc=1)
	plt.ylim( (-7.0, 9.0) )
	ax.set_yticks(np.arange(-6.0,10.0,2.0))
	plt.grid()
	plt.savefig("pd_test__fig_07_real_and_imag_z__with_y_minus_1.png")	
	...
	plt.xlabel("$x$")
	plt.ylabel("$\\Re(Z')$,$\\Im(Z')$ ")
	plt.plot( x_values, pdp_r__m1, label="$\\Re(Z')$"); 
	plt.plot( x_values, pdp_i__m1, label="$\\Im(Z')$");
	plt.axhline( y=0, color='black')
	plt.legend( loc=4)
	plt.ylim( (-20.0, 10.0) )
	ax.set_yticks(np.arange(-20.0,12.0,4.0))
	plt.grid()
	plt.savefig("pd_test__fig_08_real_and_imag_zprime__with_y_minus_1.png")
	
	# do figs 9 and 10
	(  x_values, pd_r__m12, pdp_r__m12, pd_i__m12, pdp_i__m12) = test_plasma_dispersion__generate_values_for_a_given_y(-1.2)
	plt.clf()
	ax = plt.subplot(111)
	plt.title("$\\Re(Z)$ and $\\Im(Z)$ versus $x$ for $y=-1.2$")
	plt.xlabel("$x$")
	plt.ylabel("$\\Re(Z)$,$\\Im(Z)$ ")
	plt.plot( x_values, pd_r__m12, label="$\\Re(Z)$"); 
	plt.plot( x_values, pd_i__m12, label="$\\Im(Z)$");
	plt.axhline( y=0, color='black')
	plt.legend( loc=1)
	plt.ylim( (-12.0, 16.0) )
	ax.set_yticks(np.arange(-12.0,20.0,4.0))
	plt.grid()
	plt.savefig("pd_test__fig_09_real_and_imag_z__with_y_minus_1.2.png")	
	...
	plt.xlabel("$x$")
	plt.ylabel("$\\Re(Z')$,$\\Im(Z')$ ")
	plt.plot( x_values, pdp_r__m12, label="$\\Re(Z')$"); 
	plt.plot( x_values, pdp_i__m12, label="$\\Im(Z')$");
	plt.axhline( y=0, color='black')
	plt.legend( loc=4)
	plt.ylim( (-40.0, 20.0) )
	ax.set_yticks(np.arange(-40.0,24.0,8.0))
	plt.grid()
	plt.savefig("pd_test__fig_10_real_and_imag_zprime__with_y_minus_1.2.png")	
...
def plot_landau_dmapings( vth_e, wp_e = 1.0):
	...
	plt.clf()
	plt.title( "landau reals")
	plt.stem( ks, reals)
	plt.savefig("landau_reals_vth_%f.png" % vth_e, dpi=200)
	plt.clf()
	plt.title( "landau imags")
	plt.stem( ks, imags)
	plt.savefig("landau_imags_vth_%f.png" % vth_e, dpi=200)
